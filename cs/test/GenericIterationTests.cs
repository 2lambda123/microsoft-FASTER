...
internal class GenericIterationTests {
  ...
  private ClientSession<MyKey, MyValue, MyInput, MyOutput, int,
                        MyFunctionsDelete> session;
  ...
  private void InternalSetup(ScanIteratorType scanIteratorType,
                             bool largeMemory) {
    // Default lockingMode for this iterator type.
    var lockingMode = scanIteratorType == ScanIteratorType.Pull
                          ? LockingMode.None
  ...
  private void InternalSetup(LockingMode lockingMode, bool largeMemory) {
    ...
    log = Devices.CreateLogDevice(
        MethodTestDir + "/GenericIterationTests.log", deleteOnClose: true);
    objlog = Devices.CreateLogDevice(MethodTestDir +
                                         "/GenericIterationTests.obj.log",
                                     deleteOnClose: true);

    fht = new FasterKV<MyKey, MyValue>(
        128,
        logSettings: new LogSettings { LogDevice = log,
                                       ObjectLogDevice = objlog,
                                       MutableFraction = 0.1,
                                       MemorySizeBits = largeMemory ? 25 : 14,
                                       PageSizeBits = largeMemory ? 20 : 9 },
        serializerSettings: new SerializerSettings<
            MyKey, MyValue> { keySerializer = () => new MyKeySerializer(),
  ...
  internal struct GenericPushIterationTestFunctions
      : IScanIteratorFunctions<MyKey, MyValue> {
    internal int keyMultToValue;
    internal long numRecords;
    internal int stopAt;

    public bool SingleReader(ref MyKey key, ref MyValue value,
    ...
    public bool ConcurrentReader(ref MyKey key, ref MyValue value,
                                 RecordMetadata recordMetadata,
                                 long numberOfRecords) =>
  ...
  public void
  GenericIterationBasicTest([Values] ScanIteratorType scanIteratorType) {
  ...
  [Category(FasterKVTestCategory)]
  [Category(SmokeTestCategory)]
  public unsafe void GenericIterationPushLockTest(
    ...
    void LocalScan(int i) {
      using var session =
          fht.For(new MyFunctionsDelete()).NewSession<MyFunctionsDelete>();
      GenericPushIterationTestFunctions scanIteratorFunctions = new();
