...
internal class BlittableIterationTests {
  ...
  internal struct BlittablePushIterationTestFunctions
      : IScanIteratorFunctions<KeyStruct, ValueStruct> {
    internal int keyMultToValue;
    internal long numRecords;
    internal int stopAt;

    public bool SingleReader(ref KeyStruct key, ref ValueStruct value,
    ...
    public bool ConcurrentReader(ref KeyStruct key, ref ValueStruct value,
                                 RecordMetadata recordMetadata,
                                 long numberOfRecords) =>
  ...
  public void
  BlittableIterationBasicTest([Values] DeviceType deviceType,
    ...
    using var session =
        fht.For(new FunctionsCompaction()).NewSession<FunctionsCompaction>();
  ...
  [Category(FasterKVTestCategory)]
  [Category(SmokeTestCategory)]
  public unsafe void BlittableIterationPushLockTest(
    ...
    fht = new FasterKV<KeyStruct, ValueStruct>(
        ...
        // locking
        new LogSettings { LogDevice = log, MemorySizeBits = 25,
                          PageSizeBits = 20, SegmentSizeBits = 22 },
    ...
    void LocalUpdate(int tid) {
      ...
      for (int i = 0; i < totalRecords; i++) {
        var key1 = new KeyStruct { kfield1 = i, kfield2 = i + 1 };
        var value =
            new ValueStruct { vfield1 = (tid + 1) * i, vfield2 = i + 1 };
