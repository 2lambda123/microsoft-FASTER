...
internal class VarLenBlittableIterationTests {
  ...
  // Note: We always set value.length to 2, which includes both VLValue members;
  // we are not exercising the "Variable Length" aspect here.
  ...
  public void Setup() {
    ...
    // Clean up log files from previous test runs in case they weren't cleaned
    // up
  ...
  internal struct VarLenBlittablePushIterationTestFunctions
      : IScanIteratorFunctions<Key, VLValue> {
    internal int keyMultToValue;
    internal long numRecords;
    internal int stopAt;

    public bool SingleReader(ref Key key, ref VLValue value,
    ...
    public bool ConcurrentReader(ref Key key, ref VLValue value,
                                 RecordMetadata recordMetadata,
                                 long numberOfRecords) =>
  ...
  public void VarLenBlittableIterationBasicTest(
      [Values] DeviceType deviceType,
      [Values] ScanIteratorType scanIteratorType) {
    log = CreateTestDevice(deviceType, $"{path}{deviceType}.log");
    fht = new FasterKV<Key, VLValue>(
        1L << 20,
        new LogSettings { LogDevice = log, MemorySizeBits = 15,
                          PageSizeBits = 9, SegmentSizeBits = 22 },
        lockingMode: scanIteratorType == ScanIteratorType.Pull
            ? LockingMode.None
            : LockingMode.Standard);
    ...
    void iterateAndVerify(int keyMultToValue, int expectedRecs) {
      ...
      if (scanIteratorType == ScanIteratorType.Pull) {
        ...
        while (iter.GetNext(out var recordInfo))
          scanIteratorFunctions.SingleReader(
              ref iter.GetKey(), ref iter.GetValue(), default, default);
      } else
        Assert.IsTrue(
            session.Iterate(ref scanIteratorFunctions),
            $"Failed to complete push iteration; numRecords = {scanIteratorFunctions.numRecords}");

      Assert.AreEqual(expectedRecs, scanIteratorFunctions.numRecords);
    }

    // Note: We always set value.length to 2, which includes both VLValue
    // members; we are not exercising the "Variable Length" aspect here.
  ...
  [Category(FasterKVTestCategory)]
  [Category(SmokeTestCategory)]
  public void VarLenBlittableIterationPushStopTest(
      [Values] DeviceType deviceType,
      [Values] ScanIteratorType scanIteratorType) {
    log = CreateTestDevice(deviceType, $"{path}{deviceType}.log");
    fht = new FasterKV<Key, VLValue>(
        1L << 20, new LogSettings { LogDevice = log, MemorySizeBits = 15,
                                    PageSizeBits = 9, SegmentSizeBits = 22 });
    ...
    void scanAndVerify(int stopAt, bool useScan) {
      ...
      if (useScan)
        Assert.IsFalse(
            fht.Log.Scan(ref scanIteratorFunctions, start, fht.Log.TailAddress),
            $"Failed to terminate push iteration early; numRecords = {scanIteratorFunctions.numRecords}");
      else
        Assert.IsFalse(
            session.Iterate(ref scanIteratorFunctions),
            $"Failed to terminate push iteration early; numRecords = {scanIteratorFunctions.numRecords}");
  ...
  [Category(FasterKVTestCategory)]
  [Category(SmokeTestCategory)]
  public unsafe void VarLenBlittableIterationPushLockTest(
      [Values(1, 4)] int scanThreads, [Values(1, 4)] int updateThreads,
      [Values] LockingMode lockingMode, [Values] ScanMode scanMode) {
    log = Devices.CreateLogDevice($"{path}lock_test.log");
    fht = new FasterKV<Key, VLValue>
        // Must be large enough to contain all records in memory to exercise
        // locking
        (1L << 20, new LogSettings { LogDevice = log, MemorySizeBits = 25,
                                     PageSizeBits = 19, SegmentSizeBits = 22 });
    ...
    void LocalScan(int i) {
      ...
      if (scanMode == ScanMode.Scan)
        Assert.IsTrue(
            fht.Log.Scan(ref scanIteratorFunctions, start, fht.Log.TailAddress),
            $"Failed to complete push scan; numRecords = {scanIteratorFunctions.numRecords}");
      else
        Assert.IsTrue(
            session.Iterate(ref scanIteratorFunctions),
            $"Failed to complete push iteration; numRecords = {scanIteratorFunctions.numRecords}");
    ...
    void LocalUpdate(int tid) {
      ...
      for (int i = 0; i < totalRecords; i++) {
        var key1 = new Key { key = i };
        var value =
            new VLValue { field1 = (tid + 1) * i, length = ValueLength };
        session.Upsert(ref key1, ref value);
      }
    }
    ...
    List<Task> tasks =
        new(); // Task rather than Thread for propagation of exception.
