...
internal unsafe sealed class BlittableAllocator<Key, Value>
  ...
  private static readonly int recordInfoSize =
      Utility.GetSize(default(RecordInfo));
  ...
  public override ref Key GetKey(long physicalAddress) {
    return ref Unsafe.AsRef<Key>((byte *)physicalAddress +
                                 RecordInfo.GetLength());
  }

  public override ref Value GetValue(long physicalAddress) {
    return ref Unsafe.AsRef<Value>((byte *)physicalAddress +
                                   RecordInfo.GetLength() + keySize);
  }
  ...
  Scan(FasterKV<Key, Value> store, long beginAddress, long endAddress,
      ...
      new BlittableScanIterator<Key, Value>(store, this, beginAddress,
                                            endAddress, scanBufferingMode,
                                            epoch, logger: logger);
  ...
  Scan<TScanFunctions>(FasterKV<Key, Value> store, long beginAddress,
                       ...
                       ScanBufferingMode scanBufferingMode) {
    using BlittableScanIterator<Key, Value> iter =
  ...
  MemoryPageScan(long beginAddress, long endAddress,
    ...
    using var iter = new BlittableScanIterator<Key, Value>(
  ...
  internal void AsyncReadPagesFromDeviceToFrame<TContext>(
      ...
      IDevice objectLogDevice = null, CancellationTokenSource cts = null) {
