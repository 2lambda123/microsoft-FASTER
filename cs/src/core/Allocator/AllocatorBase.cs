...
#region Protected size definitions
  ...
  /// <summary>
  /// How many pages do we leave empty in the in-memory buffer (between 0 and
  /// BufferSize-1)
  /// </summary>
  ...
  /// <summary>
  /// Number of <see cref="LockableUnsafeContext{Key, Value, Input, Output,
  /// Context, Functions}"/> or <see cref="LockableContext{Key, Value, Input,
  /// Output, Context, Functions}"/> instances active.
  /// </summary>
...
#region Public addresses
  ...
  /// <summary>
  /// The lowest in-memory address in the log. While we hold the epoch this may
  /// be changed by other threads as part of ShiftHeadAddress, but as long as an
  /// address was >= HeadAddress while we held the epoch, it cannot be actually
  /// evicted until we release the epoch.
  /// </summary>
  public long HeadAddress;

  /// <summary>
  /// The lowest reliable in-memory address. This is set by OnPagesClosed as the
  /// highest address of the range it is starting to close; thus it leads <see
  /// cref="ClosedUntilAddress"/>. As long as we hold the epoch, records above
  /// this address will not be evicted.
  /// </summary>
  ...
  /// <summary>
  /// The highest address that has been closed by <see cref="OnPagesClosed"/>.
  /// It will catch up to <see cref="SafeHeadAddress"/> when a region is closed.
  /// </summary>
  ...
  /// <summary>
  /// Address until which we are currently closing. Used to coordinate linear
  /// closing of pages. Only one thread will be closing pages at a time.
  /// </summary>
...
#region Private page metadata
  ...
  /// <summary>
  /// Global address of the current tail (next element to be allocated from the
  /// circular buffer)
  /// </summary>
  ...
  /// <summary>
  /// The "event" to be waited on for flush completion by the initiator of an
  /// operation
  /// </summary>
...
#region Abstract methods
  ...
  /// <returns></returns>
  public virtual ref
      Value GetValue(long physicalAddress,
                     long endPhysicalAddress) => ref GetValue(physicalAddress);
  ...
  /// <returns></returns>
  public abstract (int, int) GetRecordSize<Input, FasterSession>(
      long physicalAddress, ref Input input, FasterSession fasterSession)
      where FasterSession : IVariableLengthStruct<Value, Input>;
                            ...
                            /// <returns></returns>
                            public virtual int GetRequiredRecordSize(long physicalAddress,
                            int availableBytes) => GetAverageRecordSize();
                            ...
                            /// <summary>
                            /// Get size of fixed (known) part of record on the
                            /// main log
                            /// </summary>
                            /// <returns></returns>
                            ...
                            /// <returns></returns>
                            public abstract (int,
                            int) GetInitialRecordSize<Input, FasterSession>(ref Key key,
                            ref Input input,
      ...
      where FasterSession : IVariableLengthStruct<Value, Input>;
                            ...
                            internal abstract unsafe void PopulatePage(byte * src,
                            int required_bytes,
                            long destinationPage);
                            /// <summary>
                            ...
                            /// <param name="fuzzyStartLogicalAddress">Start
                            /// address of fuzzy region, which contains old and
                            /// new version records (we use this to selectively
                            /// flush only old-version records during snapshot
                            /// checkpoint)</param>
                            protected abstract void WriteAsyncToDevice<TContext>(long startPage,
                            long flushPage,
                            int pageSize,
                            DeviceIOCompletionCallback callback,
                            PageAsyncFlushResult<TContext> result,
                            IDevice device,
                            IDevice objectLogDevice,
                            long[] localSegmentOffsets,
                                 long fuzzyStartLogicalAddress);
  ...
  AsyncFlushDeltaToDevice(long startAddress, long endAddress,
                          long prevEndAddress, long version, DeltaLog deltaLog,
                          out SemaphoreSlim completedSemaphore,
                          int throttleCheckpointFlushDelayMs) {
    logger?.LogTrace(
        "Starting async delta log flush with throttling {throttlingEnabled}",
        throttleCheckpointFlushDelayMs >= 0
            ? $"enabled ({throttleCheckpointFlushDelayMs}ms)"
            : "disabled");
    ...
    void FlushRunner() {
      ...
      // We perform delta capture under epoch protection with page-wise refresh
      // for latency reasons
      ...
      try {
        for (long p = startPage; p < endPage; p++) {
          ...
          if ((p < prevEndPage || endAddress == prevEndAddress) &&
              PageStatusIndicator[p % BufferSize].Dirty < version)
            continue;
          ...
          if (endAddress < endLogicalAddress)
            endLogicalAddress = endAddress;
          Debug.Assert(endLogicalAddress > logicalAddress);
          var endPhysicalAddress =
              physicalAddress + (endLogicalAddress - logicalAddress);
          ...
          while (physicalAddress < endPhysicalAddress) {
            ...
            var (recordSize, alignedRecordSize) =
                GetRecordSize(physicalAddress);
            if (info.Dirty) {
              info.ClearDirtyAtomic(); // there may be read locks being taken,
                                       // hence atomic
              ...
              *(int *)(destPhysicalAddress + destOffset) = alignedRecordSize;
              ...
              Buffer.MemoryCopy((void *)physicalAddress,
                                (void *)(destPhysicalAddress + destOffset),
                                alignedRecordSize, alignedRecordSize);
  ...
  internal unsafe void ApplyDelta(DeltaLog log, long startPage, long endPage,
    ...
    log.Reset();
    while (log.GetNext(out long physicalAddress, out int entryLength,
                       out var type)) {
      switch (type) {
      case DeltaLogEntryType.DELTA:
        ...
        while (physicalAddress < endAddress) {
          ...
          if (address >= startLogicalAddress && address < endLogicalAddress) {
            ...
            if (oldSize > size)
              new Span<byte>((byte *)(destination + size), oldSize - size)
                  .Clear();

            // Update with new record
            Buffer.MemoryCopy((void *)physicalAddress, (void *)destination,
                              size, size);
      ...
      case DeltaLogEntryType.CHECKPOINT_METADATA:
        if (recoverTo != -1) {
          ...
          unsafe {
            fixed(byte *m = metadata) Buffer.MemoryCopy(
                (void *)physicalAddress, m, entryLength, entryLength);
          }
  ...
  internal void MarkPageAtomic(long logicalAddress, long version) {
    var offset = (logicalAddress >> LogPageSizeBits) % BufferSize;
    Utility.MonotonicUpdate(ref PageStatusIndicator[offset].Dirty, version,
                            out _);
  }

  internal void WriteAsync<TContext>(IntPtr alignedSourceAddress,
                                     ulong alignedDestinationAddress,
                                     uint numBytesToWrite,
                                     DeviceIOCompletionCallback callback,
                                     PageAsyncFlushResult<TContext> asyncResult,
                                     IDevice device) {
    if (asyncResult.partial) {
      // Write only required bytes within the page
      int aligned_start = (int)((asyncResult.fromAddress -
                                 (asyncResult.page << LogPageSizeBits)));
      aligned_start = (aligned_start / sectorSize) * sectorSize;

      int aligned_end = (int)((asyncResult.untilAddress -
                               (asyncResult.page << LogPageSizeBits)));
      aligned_end = ((aligned_end + (sectorSize - 1)) & ~(sectorSize - 1));

      numBytesToWrite = (uint)(aligned_end - aligned_start);
      device.WriteAsync(alignedSourceAddress + aligned_start,
  ...
  /// <param name="result"></param>
  protected abstract unsafe void AsyncReadRecordObjectsToMemory(
      long fromLogical, int numBytes, DeviceIOCompletionCallback callback,
      AsyncIOContext<Key, Value> context, SectorAlignedMemory result = default);
  /// <summary>
  /// Read page (async)
  /// </summary>
  /// <typeparam name="TContext"></typeparam>
  ...
  ReadAsync<TContext>(ulong alignedSourceAddress, int destinationPageIndex,
                      uint aligned_read_length,
                      DeviceIOCompletionCallback callback,
                      PageAsyncReadResult<TContext> asyncResult, IDevice device,
                      IDevice objlogDevice);
  /// <summary>
  /// Clear page
  /// </summary>
  ...
  /// <param name="asyncResult"></param>
  protected abstract void
  WriteAsync<TContext>(long flushPage, DeviceIOCompletionCallback callback,
                       PageAsyncFlushResult<TContext> asyncResult);
  /// <summary>
  ...
  /// <returns></returns>
  protected abstract unsafe bool
  RetrievedFullRecord(byte *record, ref AsyncIOContext<Key, Value> ctx);
  ...
  /// <param name="ctx"></param>
  /// <returns></returns>
  public virtual ref Key
  GetContextRecordKey(ref AsyncIOContext<Key, Value> ctx) => ref ctx.key;
  ...
  GetContextRecordValue(ref AsyncIOContext<Key, Value> ctx) => ref ctx.value;
  ...
  /// <param name="ctx"></param>
  /// <param name="value"></param>
  public virtual void PutContext(ref AsyncIOContext<Key, Value> ctx,
                                 ref Value value) => ctx.value = value;
  ...
  /// <summary>
  /// Pull-based scan interface for HLOG; user calls GetNext() which advances
  /// through the address range.
  /// </summary>
  /// <returns>Pull Scan iterator instance</returns>
  public abstract IFasterScanIterator<Key, Value>
  Scan(FasterKV<Key, Value> store, long beginAddress, long endAddress,
       ScanBufferingMode scanBufferingMode =
           ScanBufferingMode.DoublePageBuffering);

  /// <summary>
  /// Push-based scan interface for HLOG, called from LogAccessor; scan the log
  /// given address range, calling <paramref name="scanFunctions"/> for each
  /// record.
  /// </summary>
  /// <returns>True if Scan completed; false if Scan ended early due to one of
  /// the TScanIterator reader functions returning false</returns>
  internal abstract bool
  Scan<TScanFunctions>(FasterKV<Key, Value> store, long beginAddress,
                       long endAddress, ref TScanFunctions scanFunctions,
                       ScanBufferingMode scanBufferingMode =
                           ScanBufferingMode.DoublePageBuffering)
      where TScanFunctions : IScanIteratorFunctions<Key, Value>;

                             /// <summary>
                             /// Push-based iteration of key versions, calling
                             /// <paramref name="scanFunctions"/> for each
                             /// record.
                             /// </summary>
                             /// <returns>True if Scan completed; false if Scan
                             /// ended early due to one of the TScanIterator
                             /// reader functions returning false</returns>
                             internal bool IterateKeyVersions<TScanFunctions>(FasterKV<Key, Value> store,
                             ref Key key,
                             ref TScanFunctions scanFunctions)
      where TScanFunctions : IScanIteratorFunctions<Key, Value> {
    OperationStackContext<Key, Value> stackCtx =
        new(store.comparer.GetHashCode64(ref key));
    ...
    if (stackCtx.recSrc.LogicalAddress < store.hlog.BeginAddress)
      return false;
    return IterateKeyVersions(store, ref key, stackCtx.recSrc.LogicalAddress,
                              ref scanFunctions);
  }

  /// <summary>
  /// Push-based iteration of key versions, calling <paramref
  /// name="scanFunctions"/> for each record.
  /// </summary>
  /// <returns>True if Scan completed; false if Scan ended early due to one of
  /// the TScanIterator reader functions returning false</returns>
  internal abstract bool
  IterateKeyVersions<TScanFunctions>(FasterKV<Key, Value> store, ref Key key,
                                     long beginAddress,
                                     ref TScanFunctions scanFunctions)
      where TScanFunctions : IScanIteratorFunctions<Key, Value>;
                             ...
                             /// </summary>
                             internal bool PushScanImpl<TScanFunctions, TScanIterator>(FasterKV<Key, Value> store,
                             long beginAddress,
                             long endAddress,
                             ref TScanFunctions scanFunctions,
                             TScanIterator iter)
      where TScanFunctions : IScanIteratorFunctions<Key, Value>
      where TScanIterator : IFasterScanIterator<Key, Value>,
                            IPushScanIterator<Key> {
    ...
    for (; !stop && iter.GetNext(out var recordInfo); ++numRecords) {
      try {
        // Pull Iter records are in temp storage so do not need locks, but we'll
        // call ConcurrentReader because, for example, GenericAllocator may need
        // to know the object is in that region.
        if (iter.CurrentAddress >= headAddress && !recordInfo.IsClosed)
          stop = !scanFunctions.ConcurrentReader(
              ref iter.GetKey(), ref iter.GetValue(),
              new RecordMetadata(recordInfo, iter.CurrentAddress), numRecords);
        else
          stop = !scanFunctions.SingleReader(
              ref iter.GetKey(), ref iter.GetValue(),
              new RecordMetadata(recordInfo, iter.CurrentAddress), numRecords);
      } catch (Exception ex) {
  ...
  /// </summary>
  internal bool IterateKeyVersionsImpl<TScanFunctions, TScanIterator>(
      FasterKV<Key, Value> store, ref Key key, long beginAddress,
      ref TScanFunctions scanFunctions, TScanIterator iter)
      where TScanFunctions : IScanIteratorFunctions<Key, Value>
      where TScanIterator : IFasterScanIterator<Key, Value>,
                            IPushScanIterator<Key> {
    ...
    for (; !stop && iter.BeginGetPrevInMemory(ref key, out var recordInfo,
                                              out continueOnDisk);
         ++numRecords) {
      OperationStackContext<Key, Value> stackCtx = default;
      try {
        // Iter records above headAddress will be in log memory and must be
        // locked.
        if (iter.CurrentAddress >= headAddress && !recordInfo.IsClosed) {
          store.LockForScan(ref stackCtx, ref key, ref iter.GetLockableInfo());
          stop = !scanFunctions.ConcurrentReader(
              ref key, ref iter.GetValue(),
              new RecordMetadata(recordInfo, iter.CurrentAddress), numRecords);
        } else
          stop = !scanFunctions.SingleReader(
              ref key, ref iter.GetValue(),
        ...
        if (stackCtx.recSrc.HasLock)
          store.UnlockForScan(ref stackCtx, ref iter.GetKey(),
                              ref iter.GetLockableInfo());
    ...
    if (continueOnDisk) {
      ...
      try {
        ...
        while (!stop && GetFromDiskAndPushToReader(
                            ref key, ref logicalAddress, ref scanFunctions,
                            numRecords, completionEvent, out stop))
          ++numRecords;
      } catch (Exception ex) {
        scanFunctions.OnException(ex, numRecords);
        throw;
  ...
  internal unsafe bool GetFromDiskAndPushToReader<TScanFunctions>(
      ref Key key, ref long logicalAddress, ref TScanFunctions scanFunctions,
      long numRecords,
    ...
    this.AsyncGetFromDisk(logicalAddress, this.GetAverageRecordSize(),
    ...
    RecordInfo recordInfo = this.GetInfoFromBytePointer(
        completionEvent.request.record.GetValidPointer());
    recordInfo.ClearBitsForDiskImages();
    stop = !scanFunctions.SingleReader(
        ref key, ref this.GetContextRecordValue(ref completionEvent.request),
        new RecordMetadata(recordInfo, completionEvent.request.logicalAddress),
        numRecords);
  ...
  MemoryPageScan(long beginAddress, long endAddress,
                 IObserver<IFasterScanIterator<Key, Value>> observer);
#endregion
  ...
  public AllocatorBase(LogSettings settings,
                       ...
                       Action<long, long> evictCallback, LightEpoch epoch,
                       Action<CommitInfo> flushCallback,
                       ILogger logger = null) {
    this.logger = logger;
    if (settings.LogDevice == null) {
      throw new FasterException(
          "LogSettings.LogDevice needs to be specified (e.g., use " +
          "Devices.CreateLogDevice, AzureStorageDevice, or NullDevice)");
    }
    ...
    SegmentBufferSize = 1 + (LogTotalSizeBytes / SegmentSize < 1
                                 ? 1
                                 : (int)(LogTotalSizeBytes / SegmentSize));
    ...
    if ((LogTotalSizeBits != 0) && (LogTotalSizeBytes < PageSize))
      throw new FasterException(
          $"Memory size ({LogTotalSizeBytes}) must be configured to be either 1 (i.e., 0 bits) or at least page size ({PageSize})");
  ...
  /// <summary>
  /// Reset the hybrid log. WARNING: assumes that threads have drained out at
  /// this point.
  /// </summary>
  ...
  internal void VerifyRecoveryInfo(HybridLogCheckpointInfo recoveredHLCInfo,
                                   bool trimLog = false) {
    // Note: trimLog is unused right now. Can be used to trim the log to the
    // minimum segment range necessary for recovery to given checkpoint
    ...
    logger?.LogInformation("Recovery requires disk segments in range " +
                           "[{firstSegment}--{tailStartSegment}]",
                           firstValidSegment, lastValidSegment);
    ...
    logger?.LogInformation("Available segment range on device: " +
                           "[{firstAvailSegment}--{lastAvailSegment}]",
                           firstAvailSegment, lastAvailSegment);
    ...
    if (trimLog) {
      logger?.LogInformation(
          "Trimming disk segments until (not including) {firstSegment}",
          firstValidSegment);
  ...
  protected void Initialize(long firstValidAddress) {
    Debug.Assert(
        firstValidAddress <= PageSize,
        $"firstValidAddress {firstValidAddress} shoulld be <= PageSize {PageSize}");
  ...
  public int EmptyPageCount {
    ...
    set {
      ...
      lock (this) // linearize all setters of EmptyPageCount
      {
        ...
        // Lag addresses are the number of pages "behind" TailPageOffset (the
        // tail in the circular buffer).
        ReadOnlyLagAddress = (long)(LogMutableFraction * headOffsetLagSize)
                             << LogPageSizeBits;
  ...
  /// <param name="page"></param>
  /// <returns></returns>
  public int GetPageIndexForPage(long page) { return (int)(page % BufferSize); }
  ...
  /// </summary>
  /// <returns></returns>
  public int GetCapacityNumPages() { return BufferSize; }
  ...
  /// <param name="address"></param>
  /// <returns></returns>
  public long GetOffsetInPage(long address) { return address & PageSizeMask; }

  /// <summary>
  /// Get sector size for main hlog device
  /// </summary>
  /// <returns></returns>
  public int GetDeviceSectorSize() { return sectorSize; }

  /// <summary>
  /// Try allocate, no thread spinning allowed
  /// </summary>
  /// <param name="numSlots">Number of slots to allocate</param>
  /// <returns>The allocated logical address, or 0 in case of inability to
  /// allocate</returns>
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  public long TryAllocate(int numSlots = 1) {
    ...
    localTailPageOffset.PageAndOffset =
        Interlocked.Add(ref TailPageOffset.PageAndOffset, numSlots);
...
#region HANDLE PAGE OVERFLOW
    if (localTailPageOffset.Offset > PageSize) {
      ...
      if (NeedToWait(pageIndex)) {
        ...
        Interlocked.Exchange(ref TailPageOffset.PageAndOffset,
                             localTailPageOffset.PageAndOffset);
        return 0; // RETRY_LATER
      }

      // The thread that "makes" the offset incorrect should allocate next page
      // and set new tail
  ...
  // If the page we are trying to allocate is past the last page with an
  // unclosed address region, then we can retry immediately because this is
  // called after NeedToWait, so we know we've completed the wait on flushEvent
  // for the necessary pages to be flushed, and are waiting for OnPagesClosed to
  // be completed.
  private bool CannotAllocate(int page) => page >= BufferSize +
                                                       (ClosedUntilAddress >>
                                                        LogPageSizeBits);

  // If the page we are trying to allocate is past the last page with an
  // unflushed address region, we have to wait for the flushEvent.
  private bool NeedToWait(int page) => page >= BufferSize +
                                                   (FlushedUntilAddress >>
                                                    LogPageSizeBits);

  /// <summary>
  /// Used by applications to make the current state of the database immutable
  /// quickly
  /// </summary>
  /// <param name="tailAddress"></param>
  /// <param name="notifyDone"></param>
  public bool ShiftReadOnlyToTail(out long tailAddress,
                                  out SemaphoreSlim notifyDone) {
    notifyDone = null;
    tailAddress = GetTailAddress();
    long localTailAddress = tailAddress;
    long currentReadOnlyOffset = ReadOnlyAddress;
    if (Utility.MonotonicUpdate(ref ReadOnlyAddress, tailAddress,
                                out long oldReadOnlyOffset)) {
  ...
  public bool ShiftReadOnlyAddress(long newReadOnlyAddress) {
    if (Utility.MonotonicUpdate(ref ReadOnlyAddress, newReadOnlyAddress,
                                out long oldReadOnlyOffset)) {
  ...
  public void ShiftBeginAddress(long newBeginAddress, bool truncateLog) {
    ...
    if (!Utility.MonotonicUpdate(ref BeginAddress, newBeginAddress,
                                 out long oldBeginAddress)) {
      if (truncateLog)
  ...
  /// <summary>
  /// Wraps <see cref="IDevice.TruncateUntilAddress(long)"/> when an allocator
  /// potentially has to interact with multiple devices
  /// </summary>
  ...
  internal virtual bool TryComplete() { return device.TryComplete(); }
  ...
  private void OnPagesMarkedReadOnly(long newSafeReadOnlyAddress) {
    if (Utility.MonotonicUpdate(ref SafeReadOnlyAddress, newSafeReadOnlyAddress,
                                out long oldSafeReadOnlyAddress)) {
      // Debug.WriteLine("SafeReadOnly shifted from {0:X} to {1:X}",
      // oldSafeReadOnlyAddress, newSafeReadOnlyAddress);
      if (OnReadOnlyObserver != null) {
        // This scan does not need a store because it does not lock; it is
        // epoch-protected so by the time it runs no current thread will have
        // seen a record below the new ReadOnlyAddress as "in mutable region".
        using var iter =
            Scan(store: null, oldSafeReadOnlyAddress, newSafeReadOnlyAddress,
  ...
  private void OnPagesClosed(long newSafeHeadAddress) {
    ...
    if (Utility.MonotonicUpdate(ref SafeHeadAddress, newSafeHeadAddress,
                                out _)) {
      // This thread is responsible for [oldSafeHeadAddress ->
      // newSafeHeadAddress]
      for (;; Thread.Yield()) {
        ...
        // We'll continue the loop if we fail the CAS here; that means another
        // thread extended the Ongoing range.
        if (Interlocked.CompareExchange(
                ref OngoingCloseUntilAddress, newSafeHeadAddress,
                _ongoingCloseUntilAddress) == _ongoingCloseUntilAddress) {
          if (_ongoingCloseUntilAddress == 0) {
            // There was no other thread running the OPCWorker loop, so this
            // thread is responsible for closing [ClosedUntilAddress ->
            // newSafeHeadAddress]
            OnPagesClosedWorker();
          } else {
            // There was another thread runnning the OPCWorker loop, and its
            // ongoing close operation was successfully extended to include the
            // new safe head address; we have no further work here.
          }
          return;
        }
      }
  ...
  private void OnPagesClosedWorker() {
    for (;; Thread.Yield()) {
      ...
      for (long closePageAddress = closeStartAddress & ~PageSizeMask;
           closePageAddress < closeEndAddress; closePageAddress += PageSize) {
        long start = closeStartAddress > closePageAddress ? closeStartAddress
                                                          : closePageAddress;
        long end = closeEndAddress < closePageAddress + PageSize
                       ? closeEndAddress
                       : closePageAddress + PageSize;

        // This scan does not need a store because it does not lock; it is
        // epoch-protected so by the time it runs no current thread will have
        // seen a record below the eviction range as "in mutable region".
        if (OnEvictionObserver is not null)
          MemoryPageScan(start, end, OnEvictionObserver);

        // If we are using a null storage device, we must also shift
        // BeginAddress
        if (IsNullDevice)
          Utility.MonotonicUpdate(ref BeginAddress, end, out _);

        // If the end of the closing range is at the end of the page, free the
        // page
      ...
      if (Interlocked.CompareExchange(ref OngoingCloseUntilAddress, 0,
                                      closeEndAddress) == closeEndAddress)
        break;
    }
  }
  ...
  private void DebugPrintAddresses() {
    ...
    Console.WriteLine("BeginAddress: {0}.{1}", GetPage(_begin),
                      GetOffsetInPage(_begin));
    Console.WriteLine("ClosedUntilAddress: {0}.{1}", GetPage(_closedUntil),
                      GetOffsetInPage(_closedUntil));
    Console.WriteLine("SafeHead: {0}.{1}", GetPage(_safehead),
                      GetOffsetInPage(_safehead));
    Console.WriteLine("Head: {0}.{1}", GetPage(_head), GetOffsetInPage(_head));
    Console.WriteLine("FlushedUntil: {0}.{1}", GetPage(_flush),
                      GetOffsetInPage(_flush));
    Console.WriteLine("SafeReadOnly: {0}.{1}", GetPage(_safereadonly),
                      GetOffsetInPage(_safereadonly));
    Console.WriteLine("ReadOnly: {0}.{1}", GetPage(_readonly),
                      GetOffsetInPage(_readonly));
  ...
  private void PageAlignedShiftReadOnlyAddress(long currentTailAddress) {
    ...
    if (Utility.MonotonicUpdate(ref ReadOnlyAddress, desiredReadOnlyAddress,
                                out long oldReadOnlyAddress)) {
      // Debug.WriteLine("Allocate: Moving read-only offset from {0:X} to
      // {1:X}", oldReadOnlyAddress, desiredReadOnlyAddress);
      epoch.BumpCurrentEpoch(() =>
                                 OnPagesMarkedReadOnly(desiredReadOnlyAddress));
    }
  }

  /// <summary>
  /// Called whenever a new tail page is allocated or when the user is checking
  /// for a failed memory allocation Tries to shift head address based on the
  /// head offset lag size.
  /// </summary>
  /// <param name="currentTailAddress"></param>
  private void PageAlignedShiftHeadAddress(long currentTailAddress) =>
      ShiftHeadAddress((currentTailAddress & ~PageSizeMask) -
                       HeadOffsetLagAddress);
  ...
  public long ShiftHeadAddress(long desiredHeadAddress) {
    ...
    if (Utility.MonotonicUpdate(ref HeadAddress, newHeadAddress,
                                out long oldHeadAddress)) {
      // Debug.WriteLine("Allocate: Moving head offset from {0:X} to {1:X}",
      // oldHeadAddress, newHeadAddress);
  ...
  /// <summary>
  /// Every async flush callback tries to update the flushed until address to
  /// the latest value possible Is there a better way to do this with enabling
  /// fine-grained addresses (not necessarily at page boundaries)?
  /// </summary>
  protected void ShiftFlushedUntilAddress() {
    ...
    long pageLastFlushedAddress =
        PageStatusIndicator[page % BufferSize].LastFlushedUntilAddress;
    while (pageLastFlushedAddress >= currentFlushedUntilAddress &&
           currentFlushedUntilAddress >= (page << LogPageSizeBits)) {
      currentFlushedUntilAddress = pageLastFlushedAddress;
      update = true;
      page++;
      pageLastFlushedAddress =
          PageStatusIndicator[page % BufferSize].LastFlushedUntilAddress;
    }

    if (update) {
      // Anything here must be valid flushes because error flushes do not set
      // LastFlushedUntilAddress, which prevents future ranges from being marked
      // as flushed
      if (Utility.MonotonicUpdate(ref FlushedUntilAddress,
                                  currentFlushedUntilAddress,
                                  out long oldFlushedUntilAddress)) {
        FlushCallback?.Invoke(
            new CommitInfo { FromAddress = oldFlushedUntilAddress,
                             UntilAddress = currentFlushedUntilAddress,
                             ErrorCode = 0 });

        this.FlushEvent.Set();

        if ((oldFlushedUntilAddress < notifyFlushedUntilAddress) &&
            (currentFlushedUntilAddress >= notifyFlushedUntilAddress)) {
    ...
    if (!errorList.Empty) {
      ...
      if (info.FromAddress == FlushedUntilAddress) {
        // all requests before error range has finished successfully -- this is
        // the earliest error and we can invoke callback on it.
  ...
  public void RecoveryReset(long tailAddress, long headAddress,
                            long beginAddress, long readonlyAddress) {
    ...
    logger?.LogInformation("Safe Head Address: {SafeHeadAddress}",
                           SafeHeadAddress);
    logger?.LogInformation("ReadOnly Address: {ReadOnlyAddress}",
                           ReadOnlyAddress);
    logger?.LogInformation("Safe ReadOnly Address: {SafeReadOnlyAddress}",
                           SafeReadOnlyAddress);
    logger?.LogInformation("Tail Address: {tailAddress}", tailAddress);
  }

  /// <summary>
  /// Invoked by users to obtain a record from disk. It uses sector aligned
  /// memory to read the record efficiently into memory.
  /// </summary>
  /// <param name="fromLogical"></param>
  /// <param name="numBytes"></param>
  /// <param name="callback"></param>
  ...
  AsyncReadRecordToMemory(long fromLogical, int numBytes,
                          DeviceIOCompletionCallback callback,
                          ref AsyncIOContext<Key, Value> context) {
    ulong fileOffset =
        (ulong)(AlignedPageSizeBytes * (fromLogical >> LogPageSizeBits) +
                (fromLogical & PageSizeMask));
    ulong alignedFileOffset =
        (ulong)(((long)fileOffset / sectorSize) * sectorSize);

    uint alignedReadLength =
        (uint)((long)fileOffset + numBytes - (long)alignedFileOffset);
    alignedReadLength =
        (uint)((alignedReadLength + (sectorSize - 1)) & ~(sectorSize - 1));

    var record = bufferPool.Get((int)alignedReadLength);
    record.valid_offset = (int)(fileOffset - alignedFileOffset);
    record.available_bytes =
        (int)(alignedReadLength - (fileOffset - alignedFileOffset));
    record.required_bytes = numBytes;

    var asyncResult =
        default(AsyncGetFromDiskResult<AsyncIOContext<Key, Value>>);
    ...
    device.ReadAsync(alignedFileOffset,
                     ...
                     alignedReadLength, callback, asyncResult);
  }
  ...
  AsyncReadRecordToMemory(long fromLogical, int numBytes,
    ...
    context.record.available_bytes =
        (int)(alignedReadLength - (fileOffset - alignedFileOffset));
    context.record.required_bytes = numBytes;

    device.ReadAsync(alignedFileOffset, (IntPtr)context.record.aligned_pointer,
                     alignedReadLength, callback, context);
  }
  ...
  public void AsyncReadPagesFromDevice<TContext>(
      long readPageStart, int numPages, long untilAddress,
      DeviceIOCompletionCallback callback, TContext context,
      long devicePageOffset = 0, IDevice logDevice = null,
      IDevice objectLogDevice = null) {
    AsyncReadPagesFromDevice(readPageStart, numPages, untilAddress, callback,
                             context, out _, devicePageOffset, logDevice,
                             objectLogDevice);
  }
  ...
  private void AsyncReadPagesFromDevice<TContext>(
      long readPageStart, int numPages, long untilAddress,
      DeviceIOCompletionCallback callback, TContext context,
      out CountdownEvent completed, long devicePageOffset = 0,
    ...
    for (long readPage = readPageStart; readPage < (readPageStart + numPages);
         readPage++) {
      ...
      var asyncResult = new PageAsyncReadResult<TContext>() {
        page = readPage, offset = devicePageOffset, context = context,
        handle = completed, maxPtr = PageSize
      };

      ulong offsetInFile = (ulong)(AlignedPageSizeBytes * readPage);
      uint readLength = (uint)AlignedPageSizeBytes;
      long adjustedUntilAddress =
          AlignedPageSizeBytes * (untilAddress >> LogPageSizeBits) +
          (untilAddress & PageSizeMask);

      if (adjustedUntilAddress > 0 &&
          ((adjustedUntilAddress - (long)offsetInFile) < PageSize)) {
        readLength = (uint)(adjustedUntilAddress - (long)offsetInFile);
        asyncResult.maxPtr = readLength;
        readLength =
            (uint)((readLength + (sectorSize - 1)) & ~(sectorSize - 1));
      }

      if (device != null)
        offsetInFile =
            (ulong)(AlignedPageSizeBytes * (readPage - devicePageOffset));

      ReadAsync(offsetInFile, pageIndex, readLength, callback, asyncResult,
                usedDevice, usedObjlogDevice);
    }
  }
  ...
  public void AsyncFlushPages(long fromAddress, long untilAddress) {
     ...
     */
    for (long flushPage = startPage; flushPage < (startPage + numPages);
         flushPage++) {
      ...
      var asyncResult = new PageAsyncFlushResult<Empty> {
        page = flushPage, count = 1, partial = false,
        fromAddress = pageStartAddress, untilAddress = pageEndAddress
      };
      if (((fromAddress > pageStartAddress) &&
           (fromAddress < pageEndAddress)) ||
          ((untilAddress > pageStartAddress) &&
      ...
      if (asyncResult.untilAddress <= BeginAddress) {
        ...
        Utility.MonotonicUpdate(ref PageStatusIndicator[flushPage % BufferSize]
                                    .LastFlushedUntilAddress,
                                BeginAddress, out _);
      ...
      if (IsNullDevice) {
        ...
        Utility.MonotonicUpdate(ref PageStatusIndicator[flushPage % BufferSize]
                                    .LastFlushedUntilAddress,
                                asyncResult.untilAddress, out _);
        ShiftFlushedUntilAddress();
        continue;
      }
      ...
      if (GetOffsetInPage(asyncResult.fromAddress) > 0) {
        ...
        // Try to merge request with existing adjacent (earlier) pending
        // requests
        while (PendingFlush[index].RemovePreviousAdjacent(
            asyncResult.fromAddress, out var existingRequest)) {
        ...
        if (PendingFlush[index].RemoveNextAdjacent(
                FlushedUntilAddress, out PageAsyncFlushResult<Empty> request)) {
          WriteAsync(request.fromAddress >> LogPageSizeBits,
                     AsyncFlushPageCallback, request);
        }
      } else
  ...
  /// <param name="callback"></param>
  /// <param name="context"></param>
  public void AsyncFlushPages<TContext>(long flushPageStart, int numPages,
                                        DeviceIOCompletionCallback callback,
                                        TContext context) {
    for (long flushPage = flushPageStart;
         flushPage < (flushPageStart + numPages); flushPage++) {
      int pageIndex = GetPageIndexForPage(flushPage);
      var asyncResult = new PageAsyncFlushResult<TContext>() {
        page = flushPage, context = context, count = 1, partial = false,
  ...
  /// <param name="completedSemaphore"></param>
  /// <param name="throttleCheckpointFlushDelayMs"></param>
  public void AsyncFlushPagesToDevice(long startPage, long endPage,
                                      long endLogicalAddress,
                                      long fuzzyStartLogicalAddress,
                                      IDevice device, IDevice objectLogDevice,
                                      out SemaphoreSlim completedSemaphore,
                                      int throttleCheckpointFlushDelayMs) {
    logger?.LogTrace(
        "Starting async delta log flush with throttling {throttlingEnabled}",
    ...
    void FlushRunner() {
      ...
      var flushCompletionTracker = new FlushCompletionTracker(
          _completedSemaphore,
          throttleCheckpointFlushDelayMs >= 0 ? new SemaphoreSlim(0) : null,
          totalNumPages);
      ...
      for (long flushPage = startPage; flushPage < endPage; flushPage++) {
        ...
        var asyncResult = new PageAsyncFlushResult<Empty> {
          flushCompletionTracker = flushCompletionTracker, page = flushPage,
          fromAddress = flushPageAddress,
          untilAddress = flushPageAddress + pageSize, count = 1
        };

        // Intended destination is flushPage
        WriteAsyncToDevice(startPage, flushPage, pageSize,
                           AsyncFlushPageToDeviceCallback, asyncResult, device,
                           objectLogDevice, localSegmentOffsets,
                           fuzzyStartLogicalAddress);
  ...
  internal void AsyncGetFromDisk(long fromLogical, int numBytes,
    ...
    if (result == null)
      AsyncReadRecordToMemory(fromLogical, numBytes, AsyncGetFromDiskCallback,
                              ref context);
    else
      AsyncReadRecordObjectsToMemory(fromLogical, numBytes,
                                     AsyncGetFromDiskCallback, context, result);
  }

  private unsafe void AsyncGetFromDiskCallback(uint errorCode, uint numBytes,
                                               object context) {
    if (errorCode != 0)
    ...
    try {
      ...
      int requiredBytes =
          GetRequiredRecordSize((long)record, ctx.record.available_bytes);
      if (ctx.record.available_bytes >= requiredBytes) {
        // We have all the required bytes. If we don't have the complete record,
        // RetrievedFullRecord calls AsyncGetFromDisk.
        if (!RetrievedFullRecord(record, ref ctx))
          return;

        // If request_key is null we're called from ReadAtAddress, so it is an
        // implicit match.
        if (ctx.request_key is not null &&
            !comparer.Equals(ref ctx.request_key.Get(),
                             ref GetContextRecordKey(ref ctx))) {
          // Keys don't match so request the previous record in the chain if it
          // is in the range to resolve.
          ctx.logicalAddress = GetInfoFromBytePointer(record).PreviousAddress;
          if (ctx.logicalAddress >= BeginAddress &&
              ctx.logicalAddress >= ctx.minAddress) {
            ctx.record.Return();
        ...
        // Either the keys match or we are below the range to retrieve (which
        // ContinuePending* will detect), so we're done.
  ...
  /// <param name="numBytes"></param>
  /// <param name="context"></param>
  private void AsyncFlushPageCallback(uint errorCode, uint numBytes,
                                      object context) {
    try {
      if (errorCode != 0) {
      ...
      if (Interlocked.Decrement(ref result.count) == 0) {
        if (errorCode != 0) {
          // Note down error details and trigger handling only when we are
          // certain this is the earliest error among currently issued flushes
          errorList.Add(new CommitInfo { FromAddress = result.fromAddress,
                                         UntilAddress = result.untilAddress,
                                         ErrorCode = errorCode });
        } else {
          // Update the page's last flushed until address only if there is no
          // failure.
          Utility.MonotonicUpdate(
              ref PageStatusIndicator[result.page % BufferSize]
                  .LastFlushedUntilAddress,
      ...
      var _flush = FlushedUntilAddress;
      if (GetOffsetInPage(_flush) > 0 &&
          PendingFlush[GetPage(_flush) % BufferSize].RemoveNextAdjacent(
              _flush, out PageAsyncFlushResult<Empty> request)) {
        WriteAsync(request.fromAddress >> LogPageSizeBits,
                   AsyncFlushPageCallback, request);
      }
    } catch when (disposed) {
  ...
  /// <param name="numBytes"></param>
  /// <param name="context"></param>
  protected void AsyncFlushPageToDeviceCallback(uint errorCode, uint numBytes,
                                                object context) {
    try {
      if (errorCode != 0) {
        logger?.LogError("AsyncFlushPageToDeviceCallback error: {0}",
                         errorCode);
      }
