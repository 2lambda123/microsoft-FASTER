...
internal unsafe sealed class GenericAllocator<Key, Value>
    : AllocatorBase<Key, Value> {
  ...
  private static readonly int recordSize =
      Utility.GetSize(default(Record<Key, Value>));
  ...
  public GenericAllocator(LogSettings settings,
                          SerializerSettings<Key, Value> serializerSettings,
                          IFasterEqualityComparer<Key> comparer,
                          Action<long, long> evictCallback = null,
                          LightEpoch epoch = null,
                          Action<CommitInfo> flushCallback = null,
                          ILogger logger = null)
    ...
    if (settings.ObjectLogDevice == null) {
      throw new FasterException(
          "LogSettings.ObjectLogDevice needs to be specified (e.g., use " +
    ...
    if ((!keyBlittable) && (settings.LogDevice as NullDevice == null) &&
        ((SerializerSettings == null) ||
         (SerializerSettings.keySerializer == null))) {
#if DEBUG
      if (typeof(Key) != typeof(byte[]) && typeof(Key) != typeof(string))
        Debug.WriteLine("Key is not blittable, but no serializer specified " +
                        "via SerializerSettings. Using (slow) " +
                        "DataContractSerializer as default.");
#endif
      SerializerSettings.keySerializer = ObjectSerializer.Get<Key>();
    }

    if ((!valueBlittable) && (settings.LogDevice as NullDevice == null) &&
        ((SerializerSettings == null) ||
         (SerializerSettings.valueSerializer == null))) {
#if DEBUG
      if (typeof(Value) != typeof(byte[]) && typeof(Value) != typeof(string))
        Debug.WriteLine("Value is not blittable, but no serializer specified " +
                        "via SerializerSettings. Using (slow) " +
                        "DataContractSerializer as default.");
#endif
  ...
  public override ref RecordInfo GetInfo(long physicalAddress) {
    ...
    int pageIndex =
        (int)((physicalAddress >> LogPageSizeBits) & BufferSizeMask);
  ...
  public override (int, int) GetRecordSize<Input, FasterSession>(
      long physicalAddress, ref Input input, FasterSession fasterSession) {
    return (recordSize, recordSize);
  }

  public override int GetAverageRecordSize() { return recordSize; }

  public override int GetFixedRecordSize() => recordSize;

  public override (int, int) GetInitialRecordSize<Input, FasterSession>(
      ref Key key, ref Input input, FasterSession fasterSession) {
    return (recordSize, recordSize);
  }
  ...
  public override AddressInfo *GetKeyAddressInfo(long physicalAddress) {
    return (AddressInfo *)Unsafe.AsPointer(
        ref Unsafe.AsRef<Record<Key, Value>>((byte *)physicalAddress).key);
  }

  public override AddressInfo *GetValueAddressInfo(long physicalAddress) {
    return (AddressInfo *)Unsafe.AsPointer(
        ref Unsafe.AsRef<Record<Key, Value>>((byte *)physicalAddress).value);
  }

  /// <summary>
  /// Allocate memory page, pinned in memory, and in sector aligned form, if
  /// possible
  /// </summary>
  ...
  protected override void TruncateUntilAddress(long toAddress) {
    ...
    objectLogDevice.TruncateUntilSegment(
        (int)(toAddress >> LogSegmentSizeBits));
  }
  ...
  WriteAsync<TContext>(long flushPage, DeviceIOCompletionCallback callback,
    ...
    WriteAsync(flushPage, (ulong)(AlignedPageSizeBytes * flushPage),
               (uint)PageSize, callback, asyncResult, device, objectLogDevice);
  }

  protected override void
  WriteAsyncToDevice<TContext>(long startPage, long flushPage, int pageSize,
    ...
    try {
      if (HeadAddress >= (flushPage << LogPageSizeBits) + pageSize) {
        ...
        WriteAsync(
            flushPage, (ulong)(AlignedPageSizeBytes * (flushPage - startPage)),
            (uint)pageSize, callback, asyncResult, device, objectLogDevice,
            flushPage, localSegmentOffsets, fuzzyStartLogicalAddress);
      }
    } finally {
      if (epochTaken)
        epoch.Suspend();
  ...
  internal override void ClearPage(long page, int offset) {
    Array.Clear(values[page % BufferSize], offset / recordSize,
                values[page % BufferSize].Length - offset / recordSize);
  }
  ...
  private void WriteAsync<TContext>(
      long flushPage, ulong alignedDestinationAddress, uint numBytesToWrite,
      DeviceIOCompletionCallback callback,
      PageAsyncFlushResult<TContext> asyncResult, IDevice device,
      IDevice objlogDevice, long intendedDestinationPage = -1,
      long[] localSegmentOffsets = null,
      long fuzzyStartLogicalAddress = long.MaxValue) {
    // Short circuit if we are using a null device
    if (device as NullDevice != null) {
      device.WriteAsync(IntPtr.Zero, 0, 0, numBytesToWrite, callback,
                        asyncResult);
      return;
    }
    ...
    if (aligned_start < start && (KeyHasObjects() || ValueHasObjects())) {
      ...
      if ((flushPage > 0) || (start > GetFirstValidLogicalAddress(flushPage))) {
        ...
        PageAsyncReadResult<Empty> result =
            new PageAsyncReadResult<Empty> { handle = new CountdownEvent(1) };
        device.ReadAsync(alignedDestinationAddress + (ulong)aligned_start,
                         (IntPtr)buffer.aligned_pointer + aligned_start,
                         (uint)sectorSize, AsyncReadPageCallback, result);
        result.handle.Wait();
      }
      fixed(RecordInfo *pin = &src[0].info) {
        Debug.Assert(buffer.aligned_pointer + numBytesToWrite <=
                     (byte *)Unsafe.AsPointer(ref buffer.buffer[0]) +
                         buffer.buffer.Length);

        Buffer.MemoryCopy((void *)((long)Unsafe.AsPointer(ref src[0]) + start),
                          buffer.aligned_pointer + start,
      ...
      fixed(RecordInfo *pin = &src[0].info) {
        ...
        Buffer.MemoryCopy(
            (void *)((long)Unsafe.AsPointer(ref src[0]) + aligned_start),
            buffer.aligned_pointer + aligned_start,
    ...
    for (int i = start / recordSize; i < end / recordSize; i++) {
      if (!src[i].info.Invalid) {
        ...
        if (address < fuzzyStartLogicalAddress || !src[i].info.IsInNewVersion) {
          if (KeyHasObjects()) {
            ...
            var key_address = GetKeyAddressInfo(
                (long)(buffer.aligned_pointer + i * recordSize));
          ...
          if (ValueHasObjects() && !src[i].info.Tombstone) {
            ...
            var value_address = GetValueAddressInfo(
                (long)(buffer.aligned_pointer + i * recordSize));
          ...
          ref var record = ref Unsafe.AsRef<Record<Key, Value>>(
              buffer.aligned_pointer + i * recordSize);
      ...
      if (endPosition > ObjectBlockSize || i == (end / recordSize) - 1) {
        ...
        var _alignedLength =
            (memoryStreamTotalLength + (sectorSize - 1)) & ~(sectorSize - 1);
        var _objAddr =
            Interlocked.Add(
                ref localSegmentOffsets[(long)(alignedDestinationAddress >>
                                               LogSegmentSizeBits) %
                                        SegmentBufferSize],
                _alignedLength) -
            _alignedLength;
        ...
        if (memoryStreamTotalLength > 0) {
          ...
          fixed(void *src_ = ms.GetBuffer()) Buffer.MemoryCopy(
              src_, _objBuffer.aligned_pointer, memoryStreamTotalLength,
              memoryStreamActualLength);
        }
        ...
        if (i < (end / recordSize) - 1) {
          ...
          objlogDevice.WriteAsync(
              ...
              (ulong)_objAddr, (uint)_alignedLength,
              AsyncFlushPartialObjectLogCallback<TContext>, asyncResult);
    ...
    if (asyncResult.partial) {
      var aligned_end = (int)((asyncResult.untilAddress -
                               (asyncResult.page << LogPageSizeBits)));
      aligned_end = ((aligned_end + (sectorSize - 1)) & ~(sectorSize - 1));
      numBytesToWrite = (uint)(aligned_end - aligned_start);
    }

    var alignedNumBytesToWrite =
        (uint)((numBytesToWrite + (sectorSize - 1)) & ~(sectorSize - 1));

    // Finally write the hlog page
    device.WriteAsync((IntPtr)buffer.aligned_pointer + aligned_start,
                      alignedDestinationAddress + (ulong)aligned_start,
                      alignedNumBytesToWrite, callback, asyncResult);
  }

  private void AsyncReadPageCallback(uint errorCode, uint numBytes,
                                     object context) {
    if (errorCode != 0) {
  ...
  ReadAsync<TContext>(ulong alignedSourceAddress, int destinationPageIndex,
    ...
    if (!(KeyHasObjects() || ValueHasObjects())) {
      device.ReadAsync(alignedSourceAddress,
                       (IntPtr)asyncResult.freeBuffer1.aligned_pointer,
    ...
    device.ReadAsync(alignedSourceAddress,
                     (IntPtr)asyncResult.freeBuffer1.aligned_pointer,
                     aligned_read_length,
                     AsyncReadPageWithObjectsCallback<TContext>, asyncResult);
  }
  ...
  private void AsyncFlushPartialObjectLogCallback<TContext>(uint errorCode,
                                                            uint numBytes,
                                                            object context) {
    if (errorCode != 0) {
      logger?.LogError(
          $"AsyncFlushPartialObjectLogCallback error: {errorCode}");
    }

    // Set the page status to flushed
    PageAsyncFlushResult<TContext> result =
        (PageAsyncFlushResult<TContext>)context;
    result.done.Set();
  }
  ...
  private void AsyncReadPageWithObjectsCallback<TContext>(uint errorCode,
    ...
    PageAsyncReadResult<TContext> result =
        (PageAsyncReadResult<TContext>)context;
    ...
    if (result.resumePtr < result.untilPtr) {
      ...
      Deserialize(result.freeBuffer1.GetValidPointer(), result.resumePtr,
                  result.untilPtr, src, ms);
      ms.Dispose();
    ...
    // We will now be able to process all records until (but not including)
    // untilPtr
    GetObjectInfo(result.freeBuffer1.GetValidPointer(), ref result.untilPtr,
                  result.maxPtr, ObjectBlockSize, out long startptr,
                  out long alignedLength);
    ...
    result.objlogDevice.ReadAsync(
        (int)((result.page - result.offset) >>
              (LogSegmentSizeBits - LogPageSizeBits)),
        (ulong)startptr, (IntPtr)objBuffer.aligned_pointer, (uint)alignedLength,
        AsyncReadPageWithObjectsCallback<TContext>, result);
  }
  ...
  protected override void
  AsyncReadRecordObjectsToMemory(long fromLogical, int numBytes,
                                 DeviceIOCompletionCallback callback,
    ...
    objectLogDevice.ReadAsync(
        (int)(context.logicalAddress >> LogSegmentSizeBits), alignedFileOffset,
  ...
  internal void AsyncReadPagesFromDeviceToFrame<TContext>(
      ...
      GenericFrame<Key, Value> frame, out CountdownEvent completed,
      long devicePageOffset = 0, IDevice device = null,
    ...
    for (long readPage = readPageStart; readPage < (readPageStart + numPages);
      ...
      var asyncResult = new PageAsyncReadResult<TContext>() {
        page = readPage,   context = context, handle = completed,
        maxPtr = PageSize, frame = frame,
      };
...
#region Page handlers for objects
  ...
  public void Deserialize(byte *raw, long ptr, long untilptr,
                          Record<Key, Value>[] src, Stream stream) {
    IObjectSerializer<Key> keySerializer = null;
    IObjectSerializer<Value> valueSerializer = null;
    ...
    while (ptr < untilptr) {
      ref Record<Key, Value> record =
          ref Unsafe.AsRef<Record<Key, Value>>(raw + ptr);
      ...
      if (!record.info.Invalid) {
        if (KeyHasObjects()) {
          ...
          if (start_addr == -1)
            start_addr = key_addr->Address & ~((long)sectorSize - 1);
          if (stream.Position !=
              streamStartPos + key_addr->Address - start_addr) {
            stream.Seek(streamStartPos + key_addr->Address - start_addr,
                        SeekOrigin.Begin);
          }
        ...
        if (!record.info.Tombstone) {
          if (ValueHasObjects()) {
            ...
            if (start_addr == -1)
              start_addr = value_addr->Address & ~((long)sectorSize - 1);
            if (stream.Position !=
                streamStartPos + value_addr->Address - start_addr) {
              stream.Seek(streamStartPos + value_addr->Address - start_addr,
                          SeekOrigin.Begin);
            }
  ...
  public void GetObjectInfo(byte *raw, ref long ptr, long untilptr,
                            int objectBlockSize, out long startptr,
                            out long size) {
    ...
    while (!done && (ptr < untilptr)) {
      ...
      if (!record.info.Invalid) {
        if (KeyHasObjects()) {
          ...
          // If object pointer is greater than kObjectSize from starting object
          // pointer
          if (minObjAddress != long.MaxValue &&
              (addr - minObjAddress > objectBlockSize))
            done = true;
        }
    ...
    maxObjAddress =
        (maxObjAddress + (sectorSize - 1)) & ~((long)sectorSize - 1);
  ...
  RetrievedFullRecord(byte *record, ref AsyncIOContext<Key, Value> ctx) {
    ...
    if (ctx.objBuffer == null) {
      ...
      if (endAddress - startAddress > int.MaxValue)
        throw new FasterException("Size of key-value exceeds max of 2GB: " +
                                  (endAddress - startAddress));

      if (startAddress < 0)
        startAddress = 0;

      AsyncGetFromDisk(startAddress, (int)(endAddress - startAddress), ctx,
                       ctx.record);
      return false;
    }
    ...
    ms.Seek(ctx.objBuffer.offset + ctx.objBuffer.valid_offset,
            SeekOrigin.Begin);
  ...
  public override IHeapContainer<Key>
  GetKeyContainer(ref Key key) => new StandardHeapContainer<Key>(ref key);
  public override IHeapContainer<Value> GetValueContainer(ref Value value) =>
      new StandardHeapContainer<Value>(ref value);

  public override long[] GetSegmentOffsets() { return segmentOffsets; }

  internal override void PopulatePage(byte *src, int required_bytes,
                                      long destinationPage) {
    PopulatePage(src, required_bytes, ref values[destinationPage % BufferSize]);
  }

  internal void PopulatePageFrame(byte *src, int required_bytes,
                                  Record<Key, Value>[] frame) {
    PopulatePage(src, required_bytes, ref frame);
  }

  internal void PopulatePage(byte *src, int required_bytes,
                             ref Record<Key, Value>[] destinationPage) {
    fixed(RecordInfo *pin = &destinationPage[0].info) {
      Debug.Assert(required_bytes <= recordSize * destinationPage.Length);

      Buffer.MemoryCopy(src, Unsafe.AsPointer(ref destinationPage[0]),
  ...
  /// </summary>
  /// <returns></returns>
  public override IFasterScanIterator<Key, Value>
  Scan(FasterKV<Key, Value> store, long beginAddress, long endAddress,
       ScanBufferingMode scanBufferingMode) =>
      new GenericScanIterator<Key, Value>(store, this, beginAddress, endAddress,
                                          scanBufferingMode, epoch);
  ...
  Scan<TScanFunctions>(FasterKV<Key, Value> store, long beginAddress,
                       ...
                       ScanBufferingMode scanBufferingMode) {
    using GenericScanIterator<Key, Value> iter =
        new(store, this, beginAddress, endAddress, scanBufferingMode, epoch,
            logger: logger);
    return PushScanImpl(store, beginAddress, endAddress, ref scanFunctions,
                        iter);
  }
  ...
  IterateKeyVersions<TScanFunctions>(FasterKV<Key, Value> store, ref Key key,
    ...
    using GenericScanIterator<Key, Value> iter =
        new(store, this, store.comparer, beginAddress, epoch, logger: logger);
    return IterateKeyVersionsImpl(store, ref key, beginAddress,
                                  ref scanFunctions, iter);
  }
  ...
  MemoryPageScan(long beginAddress, long endAddress,
    ...
    int end = start + count;
    using var iter = new MemoryPageScanIterator<Key, Value>(
        values[page], start, end, pageStartAddress, recordSize);
