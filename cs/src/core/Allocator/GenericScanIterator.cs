...
/// </summary>
internal sealed class GenericScanIterator<Key, Value>
    : ScanIteratorBase,
  ...
  /// Constructor
  /// </summary>
  public GenericScanIterator(FasterKV<Key, Value> store,
                             GenericAllocator<Key, Value> hlog,
  ...
  public unsafe bool GetNext(out RecordInfo recordInfo) {
    ...
    while (true) {
      ...
      if (currentAddress >= headAddress) {
        ...
        // Copy the object values from cached page memory to data members; we
        // have no ref into the log after the epoch.Suspend() (except for
        // GetLockableInfo which we know is safe). These are pointer-sized
        // shallow copies but we need to lock to ensure no value tearing inside
        // the object while copying to temp storage.
        ...
        try {
          ...
          if (currentAddress >= headAddress && store is not null)
            store.LockForScan(ref stackCtx,
                              ref hlog.values[currentPage][currentOffset].key,
                              ref hlog.values[currentPage][currentOffset].info);
          ...
          if (stackCtx.recSrc.HasLock)
            store.UnlockForScan(
                ref stackCtx, ref hlog.values[currentPage][currentOffset].key,
                ref hlog.values[currentPage][currentOffset].info);
        }
      ...
      currentPage = currentOffset =
          -1; // We should no longer use these except for GetLockableInfo()
  ...
  bool IPushScanIterator<Key>.BeginGetPrevInMemory(ref Key key,
    ...
    while (true) {
      ...
      if (recordInfo.SkipOnScan || recordInfo.IsNull() ||
          !comparer.Equals(ref hlog.values[currentPage][currentOffset].key,
                           ref key)) {
        epoch?.Suspend();
        continue;
      }

      // Copy the object values from cached page memory to data members; we have
      // no ref into the log after the epoch.Suspend() (except for
      // GetLockableInfo which we know is safe). These are pointer-sized shallow
      // copies.
      recordInfo = hlog.values[currentPage][currentOffset].info;
      currentKey = hlog.values[currentPage][currentOffset].key;
      currentValue = hlog.values[currentPage][currentOffset].value;
  ...
  private unsafe void AsyncReadPagesCallback(uint errorCode, uint numBytes,
    ...
    if (result.freeBuffer1 != null) {
      hlog.PopulatePage(result.freeBuffer1.GetValidPointer(),
                        result.freeBuffer1.required_bytes,
                        ref frame.GetPage(result.page % frame.frameSize));
