...
internal unsafe sealed class VariableLengthBlittableAllocator<Key, Value>
    : AllocatorBase<Key, Value> {
  public const int kRecordAlignment =
      8; // RecordInfo has a long field, so it should be aligned to 8-bytes
  ...
  public VariableLengthBlittableAllocator(
      LogSettings settings, VariableLengthStructSettings<Key, Value> vlSettings,
      IFasterEqualityComparer<Key> comparer,
      Action<long, long> evictCallback = null, LightEpoch epoch = null,
      Action<CommitInfo> flushCallback = null, ILogger logger = null)
...
#endif
        pointer = pointers[index], value = values[index]
      });
      values[index] = null;
      pointers[index] = 0;
#if !NET5_0_OR_GREATER
  ...
  public override ref Key GetKey(long physicalAddress) {
    return ref KeyLength.AsRef((byte *)physicalAddress +
  ...
  private long ValueOffset(long physicalAddress) =>
      physicalAddress + RecordInfo.GetLength() +
      AlignedKeySize(physicalAddress);
  ...
  public override (int, int) GetRecordSize<Input, FasterSession>(
    ...
    var size =
        RecordInfo.GetLength() + AlignedKeySize(physicalAddress) +
        fasterSession.GetLength(ref GetValue(physicalAddress), ref input);
    return (size, (size + kRecordAlignment - 1) & (~(kRecordAlignment - 1)));
  }

  public override int GetRequiredRecordSize(long physicalAddress,
                                            int availableBytes) {
    // We need at least [record size] + [average key size] + [average value
    // size]
  ...
  public override int GetAverageRecordSize() {
    return RecordInfo.GetLength() +
           ((KeyLength.GetInitialLength() + kRecordAlignment - 1) &
            (~(kRecordAlignment - 1))) +
           ((ValueLength.GetInitialLength() + kRecordAlignment - 1) &
            (~(kRecordAlignment - 1)));
  }
  ...
  public override (int, int) GetInitialRecordSize<TInput, FasterSession>(
      ref Key key, ref TInput input, FasterSession fasterSession) {
    var actualSize = RecordInfo.GetLength() +
                     ((KeyLength.GetLength(ref key) + kRecordAlignment - 1) &
                      (~(kRecordAlignment - 1))) +
                     fasterSession.GetInitialLength(ref input);

    return (actualSize,
            (actualSize + kRecordAlignment - 1) & (~(kRecordAlignment - 1)));
  }
  ...
  public override void Serialize(ref Key src, long physicalAddress) {
    KeyLength.Serialize(ref src,
                        (byte *)physicalAddress + RecordInfo.GetLength());
  }
  ...
  public override long GetPhysicalAddress(long logicalAddress) {
    ...
    int pageIndex =
        (int)((logicalAddress >> LogPageSizeBits) & (BufferSize - 1));
  ...
  internal override void ClearPage(long page, int offset) {
    if (offset == 0)
      Array.Clear(values[page % BufferSize], offset,
                  values[page % BufferSize].Length - offset);
    else {
      // Adjust array offset for cache alignment
      offset += (int)(pointers[page % BufferSize] -
                      (long)Unsafe.AsPointer(ref values[page % BufferSize][0]));
      Array.Clear(values[page % BufferSize], offset,
                  values[page % BufferSize].Length - offset);
    }
  }
  ...
  ReadAsync<TContext>(ulong alignedSourceAddress, int destinationPageIndex,
    ...
    device.ReadAsync(alignedSourceAddress,
                     (IntPtr)pointers[destinationPageIndex],
  ...
  public override IHeapContainer<Key> GetKeyContainer(ref Key key) {
    if (fixedSizeKey)
      return new StandardHeapContainer<Key>(ref key);
    else
      return new VarLenHeapContainer<Key>(ref key, KeyLength, bufferPool);
  }

  public override IHeapContainer<Value> GetValueContainer(ref Value value) {
    if (fixedSizeValue)
      return new StandardHeapContainer<Value>(ref value);
    else
      return new VarLenHeapContainer<Value>(ref value, ValueLength, bufferPool);
  }
  ...
  /// </summary>
  /// <returns></returns>
  public override bool KeyHasObjects() { return false; }

  /// <summary>
  /// Whether KVS has values to serialize/deserialize
  /// </summary>
  /// <returns></returns>
  public override bool ValueHasObjects() { return false; }

  public override long[] GetSegmentOffsets() { return null; }
  ...
  internal override void PopulatePage(byte *src, int required_bytes,
    ...
    // Buffer.MemoryCopy(src, (void*)pointers[destinationPage % BufferSize],
  ...
  Scan(FasterKV<Key, Value> store, long beginAddress, long endAddress,
      ...
      new VariableLengthBlittableScanIterator<Key, Value>(
          store, this, beginAddress, endAddress, scanBufferingMode, epoch,
          logger: logger);
  ...
  Scan<TScanFunctions>(FasterKV<Key, Value> store, long beginAddress,
                       ...
                       ScanBufferingMode scanBufferingMode) {
    using VariableLengthBlittableScanIterator<Key, Value> iter =
  ...
  MemoryPageScan(long beginAddress, long endAddress,
    ...
    using var iter = new VariableLengthBlittableScanIterator<Key, Value>(
        store: null, this, beginAddress, endAddress,
        ScanBufferingMode.NoBuffering, epoch, true, logger: logger);
  ...
  internal void AsyncReadPagesFromDeviceToFrame<TContext>(
      ...
      BlittableFrame frame, out CountdownEvent completed,
    ...
    for (long readPage = readPageStart; readPage < (readPageStart + numPages);
      ...
      var asyncResult = new PageAsyncReadResult<TContext>() {
        page = readPage, context = context, handle = completed, frame = frame
      };
      ...
      usedDevice.ReadAsync(offsetInFile, (IntPtr)frame.pointers[pageIndex],
                           readLength, callback, asyncResult);
    }
  }
}
}
