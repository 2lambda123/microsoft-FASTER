...
/// <summary>
/// Lightweight iterator for memory page (copied to buffer). GetNext() can be
/// used outside epoch protection and locking, but ctor must be called within
/// epoch protection.
/// </summary>
...
class MemoryPageScanIterator<Key, Value> : IFasterScanIterator<Key, Value> {
  ...
  public MemoryPageScanIterator(Record<Key, Value>[] page, int start, int end,
                                long pageStartAddress, int recordSize) {
  ...
  public override string ToString() =>
      $"BA {BeginAddress}, EA {EndAddress}, CA {CurrentAddress}, NA {NextAddress}, start {start}, end {end}, recSize {recordSize}, pageSA {pageStartAddress}";
}
}
